...
def cumulative_object_counting_x_axis(input_video, detection_graph, category_index, is_color_recognition_enabled, fps, width, height, roi, deviation):
        ...
        # input video
        cap = cv2.VideoCapture(input_video)#打开摄像头或者文件
...
def cumulative_object_counting_y_axis(input_video, detection_graph, category_index, is_color_recognition_enabled, fps, width, height, roi, deviation):
        ...
        #initialize .csv
        with open('object_counting_report.csv', 'w') as f:#打开文件
                writer = csv.writer(f)  
                csv_line = "Object Type, Object Color, Object Movement Direction, Object Speed (km/h)"                 
                writer.writerows([csv_line.split(',')])

        fourcc = cv2.VideoWriter_fourcc(*'XVID')#打开视频
        output_movie = cv2.VideoWriter('the_output.avi', fourcc, fps, (width, height))#结果写入文件

        # input video
        cap = cv2.VideoCapture(input_video)#打开视频
        ...
        with detection_graph.as_default():
          with tf.Session(graph=detection_graph) as sess:
            # 确定的输入和输出 Tensors for detection_graph
            image_tensor = detection_graph.get_tensor_by_name('image_tensor:0')
            ...
            # 每个分数表示每个对象的置信水平.
            # 分数和标签一起显示在结果图像上.
            detection_scores = detection_graph.get_tensor_by_name('detection_scores:0')#分数
            detection_classes = detection_graph.get_tensor_by_name('detection_classes:0')#标签
            num_detections = detection_graph.get_tensor_by_name('num_detections:0')#检测数量
            ...
            while(cap.isOpened()):
                ...
                input_frame = frame#每一帧

                # 扩展维度，因为模型希望图像具有形状: [1, None, None, 3]
                image_np_expanded = np.expand_dims(input_frame, axis=0)
               ...
               # 检测结果的可视化.        
                ...
                # 车辆过线计数时，使ROI线颜色为绿色
                if counter == 1:                  
                  cv2.line(input_frame, (0, roi), (width, roi), (0, 0xFF, 0), 5)#绿
                else:
                  cv2.line(input_frame, (0, roi), (width, roi), (0, 0,0xFF ), 5)#红
                
                total_passed_vehicle = total_passed_vehicle + counter#这是累计结果

                # 将信息文本插入视频帧
                font = cv2.FONT_HERSHEY_SIMPLEX
                cv2.putText(#这是视频上方的统计结果
                    input_frame,
...
def object_counting(input_video, detection_graph, category_index, is_color_recognition_enabled, fps, width, height):
        #initialize .csv
        #with open('object_counting_report.csv', 'w') as f:#打开表格文件
         #       writer = csv.writer(f)  
         #       csv_line = 'Time, Person Number'#"Object Type, Object Color, Object Movement Direction, Object Speed (km/h)"                 
         #       writer.writerows([csv_line.split(',')])
          ...
          with tf.Session(graph=detection_graph) as sess:#创建对象
            ...
            while(cap.isOpened()):#如果摄像头打开
                ...
                if(len(counting_mode) != 12):
                    now_person_number = 0
                    cv2.putText(input_frame, "0 person,person_flow:{}".format(person_flow), (10, 35), font, 0.8, (0,255,255),2,cv2.FONT_HERSHEY_SIMPLEX)                       
                else:
        ...
        with detection_graph.as_default():
          with tf.Session(graph=detection_graph) as sess:
            ...
            while(cap.isOpened()):
                ...
                #output_movie.write(input_frame)####3
                #cv2.imshow('Video', input_frame)#在窗口显示图像
                print ("writing frame")
